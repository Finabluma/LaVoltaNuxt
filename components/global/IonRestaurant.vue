<script setup>
  ...
  function tlDrawSVG() {
    const tlSVG = gsap.utils.toArray('#ior path')
    const tlSVGReverse = gsap.utils.toArray('#ior path').reverse()

    function animateSVG(direction) {
      const tl = gsap.timeline()
      const scrollingDown = direction === 1
      const anim = scrollingDown ? tlSVG : tlSVGReverse

      return tl.to(anim, {
        drawSVG: () => (scrollingDown ? '0' : '100%'),
        fillOpacity: () => (scrollingDown ? '0.5' : 1),
      })
    }
    // Scrolltrigger
...
<template>
  <svg id="ior" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
    <path
